...
test('pckrPckr - remove - remove pckr from packed location', async t => {
  t.plan(0);
  const {stubs, pckrPckr } = t.context;
  const pckrPath = 'a/pckr/path';
  const packedPath = 'a/pckr/path/pckr-0.0.0.tgz';
  td.when(stubs.path.resolve(td.matchers.isA(String), '../../')).thenReturn(pckrPath);
  td.when(stubs.npm.pack(pckrPath)).thenResolve(packedPath);
  await pckrPckr.pack();
  pckrPckr.remove();
  td.verify(stubs.fs.unlinkSync(packedPath));
});

test('pckrPckr - getPath - returns pckr packed location', async t => {
  const {stubs, pckrPckr } = t.context;
  const pckrPath = 'a/pckr/path';
  const packedPath = 'a/pckr/path/pckr-0.0.0.tgz';
  td.when(stubs.path.resolve(td.matchers.isA(String), '../../')).thenReturn(pckrPath);
